...
def get_cursor(event, x, y, flags, param):
    ...
    mouseX, mouseY = x, y
    # print('global',x,y)
    cv2.setWindowTitle(window1,
                       f"Frame : {frame_number} (1:RD, 2:RU, 3:LD, 4:LU, 0:Continue Mode), Cursor(x,y)={mouseX},{mouseY}")
...
def update_key(current_key, key_list=['RD', 'RU', 'LD', 'LU']):
    if (current_key + 1) == len(key_list):
        return 0  # start of the list
    else:
...
def stride_gt_creator(video_path, out_dir):
    ...
    while cam.isOpened():
        ...
        if k == 55:  # key 7 - stf frame
            ...
            print(f"Assigning {frame_number} as STF")
    ...
    if len(total_list) > 0:
        ...
        for i in range(len(gt_strides_df) - 1):
            if (gt_strides_df['Foot'][i] == gt_strides_df['Foot'][i + 1]):
                print("Warning!!!!!!!!!!! Two consecutive foot are same")

        l = list(gt_strides_df['GU'])
        try:
            new_l = [int(i) for i in l]
        except:
            print('Cells are not complete,do it again')

        new_df = gt_strides_df
        gct_col = []
        for i in new_df.index:
            gct_col.append(new_df["GU"][i] - new_df["GD"][i])
        new_df["GCT frame"] = gct_col
        print(new_df)
